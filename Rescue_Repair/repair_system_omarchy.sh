...
detect_system() {
    ...
    if [[ ${#encrypted_parts[@]} -eq 1 ]]; then
        ...
        read -r -p "Select partition (1-${#encrypted_parts[@]}): " choice
        choice=$((choice-1))
    ...
    local fs_type
    fs_type=$(blkid -o value -s TYPE "$ENCRYPTED_PARTITION" 2>/dev/null || echo "unknown")
...
detect_filesystem() {
    ...
    if blkid /dev/mapper/"$CRYPT_NAME" | grep -q "ext4"; then
        ...
        echo "2) btrfs"
        echo "3) xfs"
        read -r -p "Choose filesystem [1-3]: " fs_choice
        case $fs_choice in
            1) FILESYSTEM="ext4" ;;
            2) FILESYSTEM="btrfs" ;;
            3) FILESYSTEM="xfs" ;;
...
mount_boot_partitions() {
    ...
    for part in "${base_device}"*; do
        if [[ "$part" != "$ENCRYPTED_PARTITION" ]]; then
            local part_label
            part_label=$(lsblk -no LABEL "$part" 2>/dev/null || echo "")
            local part_fstype
            part_fstype=$(lsblk -no FSTYPE "$part" 2>/dev/null || echo "")
...
EOF
        ...
        case $choice in
            1)
                ...
                ;;
            2)
                read -r -p "Enter encrypted partition path (e.g., /dev/nvme0n1p3): " ENCRYPTED_PARTITION
                unlock_encrypted_drive
                detect_filesystem
                mount_filesystem
                ;;
            3)
                if [[ -z "$ENCRYPTED_PARTITION" ]]; then
                    read -r -p "Enter encrypted partition path: " ENCRYPTED_PARTITION
                fi
                mount_boot_partitions
                ;;
            4)
...
reset_password() {
    ...
    read -r -p "Enter username to reset password: " username
...
backup_files() {
    ...
    read -r -p "Enter backup device path (e.g., /dev/sdb1): " backup_dev
    read -r -p "Enter backup mount point (e.g., /backup): " backup_mount
