            ...
            # Webapp match - check by URL domain if defined
                ...
                if [[ -n "$webapp_domains" ]]; then
                    ...
                    if [[ "$line" =~ ,[[:space:]]*([^,]+),[[:space:]]*exec ]]; then
                        local label
                        label=$(echo "$line" | sed -n 's/.*,[[:space:]]*\([^,]*\),[[:space:]]*exec.*/\1/p' | xargs)
                        local label_lower
...
remove_bindings_from_file() {
    ...
    # Create backup
    local backup_file
    backup_file="${BINDINGS_FILE}.backup.$(date +%Y%m%d_%H%M%S)"
...
enhanced_select_packages() {
    ...
    while IFS= read -r selected_item; do
        ...
        local clean_item
        clean_item=$(echo "$selected_item" | sed 's/^[📦🌐] //' | sed 's/ ⌨$//')
    ...
    if [[ $separator_index -ge 0 ]]; then
        ...
        if [[ "$REMOVE_BINDINGS" == true ]]; then
            ...
            for pkg in "${pkg_array[@]}"; do
                local pkg_bindings
                pkg_bindings=$(find_app_bindings "$pkg")
            ...
            for webapp in "${webapp_array[@]}"; do
                local webapp_bindings
                webapp_bindings=$(find_app_bindings "$webapp")
...
main() {
    ...
    local sel_pkgs_str="$SELECTED_PACKAGES"
    local sel_web_str="$SELECTED_WEBAPPS"
    ...
    if [[ -n "$sel_pkgs_str" ]]; then
        readarray -t packages_array <<< "$sel_pkgs_str"
    fi
    
    if [[ -n "$sel_web_str" ]]; then
        readarray -t webapps_array <<< "$sel_web_str"
    fi
    ...
    if [[ -f "$BINDINGS_FILE" ]]; then
        for pkg in "${packages_array[@]}"; do
            local bindings
            bindings=$(find_app_bindings "$pkg")
            if [[ -n "$bindings" ]]; then
        ...
        for webapp in "${webapps_array[@]}"; do
            local bindings
            bindings=$(find_app_bindings "$webapp")
            if [[ -n "$bindings" ]]; then
                selected_items_have_bindings=true
                total_bindings=$((total_bindings + $(echo "$bindings" | wc -l)))
            fi
