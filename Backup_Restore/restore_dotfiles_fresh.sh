...
echo ""
read -r -p "This will overwrite existing .config, .local and bin files. Continue? (y/n): " confirm
if [ "$confirm" != "y" ]; then
    echo "Aborted."
    exit 0
fi
...
if [ -f "${TEMP_DIR}/installed_packages_*.txt" ]; then
    PACKAGE_LIST=$(find "${TEMP_DIR}" -maxdepth 1 -name 'installed_packages_*.txt' 2>/dev/null | head -1)
