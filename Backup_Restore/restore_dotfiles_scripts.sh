...
read -r -p "Enter the absolute path to your backup archive (.tar.gz): " BACKUP_ARCHIVE_PATH
...
read -r -p "This will overwrite your current ~/.config and ~/.local/share. Do you want to proceed? (y/N): " CONFIRM_RESTORE_FILES
...
if [ -f "$PACMAN_PACKAGES_FILE" ]; then
    read -r -p "This will install/reinstall Pacman packages. Do you want to proceed? (y/N): " CONFIRM_INSTALL_PACMAN
    if [[ "$CONFIRM_INSTALL_PACMAN" =~ ^[Yy]$ ]]; then
        echo "Installing Pacman packages..."
        cat "$PACMAN_PACKAGES_FILE" | sudo pacman -S --needed - || { echo "Error: Could not install Pacman packages."; rm -rf "$RESTORE_DIR"; exit 1; }
    else
...
if [ -f "$AUR_PACKAGES_FILE" ]; then
    read -r -p "This will install/reinstall AUR packages using yay. Do you want to proceed? (y/N): " CONFIRM_INSTALL_AUR
