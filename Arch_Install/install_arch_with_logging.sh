...
partition_disk() {
    ...
    if $has_efi && $has_ntfs; then
        read -r -p "Windows installation detected on $disk. Do you want to format the entire disk? (WARNING: THIS WILL DELETE WINDOWS) (y/N) " format_disk
        if [[ "$format_disk" =~ ^[yY]$ ]]; then
            ...
            read -r -p "Found free_space_size_mbMB of free space starting at free_space_startMB. Do you want to install Arch Linux in this space? (y/N) " install_in_free
            ...
            parted -s "$disk" mkpart ESP fat32 "free_space_startMB" "efi_part_endMB"
            parted -s "$disk" set 1 esp on
            parted -s "$disk" mkpart primary btrfs "efi_part_endMB" "root_part_endMB"
        fi
    else
...
# --- Configuration ---
# shellcheck disable=SC2120,SC2119,SC2027,SC2086,SC1078,SC1079
configure_system() {
