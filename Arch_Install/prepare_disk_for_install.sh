...
create_partition() {
    ...
    read -r -p "Enter size in GB for $part_name partition (e.g., 0.5, 20, 100%, 0 to skip): " size_input
    ...
    read -r -p "Choose filesystem for $part_name [$default_fs]: " fs
    fs=${fs:-$default_fs}
    ...
    if [[ "$size_input" == "100%" ]]; then
        ...
        # Create partition
        parted -a optimal -s "$drive" mkpart "$part_name" "start_mibMiB" "end_mibMiB"
    else
        # Convert GB to MiB
        local size_mib
    ...
    echo "$part_name partition created from start_mibMiB to end_mibMiB."
...
read -r -p "Enter the full path of the drive to partition (e.g., /dev/sda or /dev/nvme0n1): " DRIVE
if [[ ! -b "$DRIVE" ]]; then
    echo "Invalid drive."
    exit 1
fi
