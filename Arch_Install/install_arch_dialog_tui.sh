...
partition_disk() {
    ...
    if [[ "$SWAP_MODE" == "partition" ]]; then
        ...
        SWAP_PART="DISKPART_PREFIX2"
        start="end_swap"
    fi

    parted --script "$DISK" mkpart root "start" 100%
    if [[ "$SWAP_MODE" == "partition" && "$BOOT_MODE" == "UEFI" ]]; then ROOT_PART="DISKPART_PREFIX3"
    elif [[ "$SWAP_MODE" == "partition" && "$BOOT_MODE" == "BIOS" ]]; then ROOT_PART="DISKPART_PREFIX3"
    elif [[ "$BOOT_MODE" == "UEFI" ]]; then ROOT_PART="DISKPART_PREFIX2"
    else ROOT_PART="DISKPART_PREFIX2"; fi
...
install_base() {
    ...
    if [[ "$SWAP_MODE" == "file" ]]; then
        arch-chroot /mnt bash -c "
            fallocate -l SWAP_SIZE_GIBG /swapfile
            chmod 600 /swapfile
            mkswap /swapfile
...
main() {
    ...
    read -ra PACKAGES <<< "$(ask_dialog "Additional packages (space-separated, optional)")"
