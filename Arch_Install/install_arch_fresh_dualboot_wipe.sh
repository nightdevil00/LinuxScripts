...
warn() { echo -e "RWARNING:N $1"; }
info() { echo -e "BINFO:N $1"; }
ok()   { echo -e "GOK:N $1"; }
...
if [[ "$MODE" == "fullwipe" ]]; then
  ...
  parted --script "$DISK" mkpart primary fat32 "EFI_START_BB" "EFI_END_BB"
  parted --script "$DISK" set "$EFI_NUM" esp on
  parted --script "$DISK" name "$EFI_NUM" "ARCH_EFI"
  parted --script "$DISK" mkpart primary btrfs "ROOT_START_BB" "ROOT_END_BB"
...
if [[ "$DISK" == *nvme* || "$DISK" == *mmcblk* ]]; then
  EFI_DEV="DISKpEFI_NUM"
  ROOT_DEV="DISKpROOT_NUM"
else
  EFI_DEV="DISKEFI_NUM"
  ROOT_DEV="DISKROOT_NUM"
fi
...
if [[ "$MODE" == "dualboot" && -n "$WIN_EFI" ]]; then
  ...
  if mount -o ro "$WIN_EFI" "$WIN_MP" 2>/dev/null; then
    ...
    if cp -r "$WIN_MP/EFI/Microsoft" /mnt/boot/EFI/ 2>/dev/null; then
      ok "Windows bootloader copied to EFI"
    else
      warn "Failed to copy Windows bootloader"
    fi
    umount "$WIN_MP"
  fi
