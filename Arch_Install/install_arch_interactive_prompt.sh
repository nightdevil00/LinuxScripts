...
read -r -p "Are you sure? This will wipe all data on it. (yes/no): " confirm
if [[ "$confirm" != "yes" ]]; then
    echo "Aborting."
    exit 1
fi
...
# Sudo access
read -r -p "Should the user have sudo access? (yes/no): " sudo_access
...
if [[ "$drive_type" == "NVMe" ]]; then
    EFI_PART="target_devicep1"
    ROOT_PART="target_devicep2"
else
    EFI_PART="target_device1"
    ROOT_PART="target_device2"
fi
