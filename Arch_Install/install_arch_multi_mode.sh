...
prompt_size() {
    local part_name=$1
    read -r -p "Enter size for $part_name (in GB): " size
    echo "$size"
}
...
list_disks
read -r -p "Enter the number of the disk: " disk_num
DISK=$(lsblk -dpno NAME | grep -v 'loop' | sed -n "disk_nump")
...
if [ -z "$PARTITIONS" ] || [ "$PARTITIONS" == "$DISK" ]; then
    ...
    read -r -p "Enter mode (1/2/3): " MODE
fi
...
if [ "$MODE" == "1" ]; then
    ...
    read -r -p "Enter root partition (to format): " ROOT_PART
    read -r -p "Enter home partition (will NOT be formatted, just mount): " HOME_PART
    ...
    sgdisk -n 0:0:+ROOT_SIZEG -t 0:8300 "$DISK"
    if [[ "$DISK" =~ nvme ]]; then
    ...
    if [ ! -z "$HOME_SIZE" ]; then
        ...
        sgdisk -n 0:0:+HOME_SIZEG -t 0:8300 "$DISK"
        if [[ "$DISK" =~ nvme ]]; then
    ...
    if [ -z "$EFI_PART" ]; then
        read -r -p "No EFI found. Create new EFI partition? (y/n): " create_efi
        if [ "$create_efi" == "y" ]; then
            EFI_SIZE=$(prompt_size "EFI")
            sgdisk -n 1:0:+EFI_SIZEG -t 1:ef00 "$DISK"
            if [[ "$DISK" =~ nvme ]]; then
                EFI_PART="DISKp1"
            else
                EFI_PART="DISK1"
    ...
    sgdisk -n 2:0:+ROOT_SIZEG -t 2:8300 "$DISK"
    sgdisk -n 3:0:+HOME_SIZEG -t 3:8300 "$DISK"
