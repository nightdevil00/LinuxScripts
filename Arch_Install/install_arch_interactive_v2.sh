...
read -r -p "Enter the disk to install Arch Linux on (e.g., /dev/sda): " DISK
read -r -p "Enter the hostname for this installation: " HOSTNAME
read -r -p "Enter the locale (e.g., en_US.UTF-8): " LOCALE
read -r -p "Enter the timezone: " TIMEZONE
read -r -p "Enter username for the new user: " USER_NAME

# Prompt for passwords
read -r -s -p "Enter password for USER_NAME: " USER_PASSWORD
echo
...
echo -n "USER_PASSWORD" | cryptsetup -y --use-random luksFormat DISK2
echo -n "USER_PASSWORD" | cryptsetup open --type luks DISK2 cryptroot
...
echo -n "USER_PASSWORD" | clevis luks bind -d /dev/disk/by-uuid/$(blkid -s UUID -o value DISK2) tpm2 '{"pcr_ids":"7"}'
