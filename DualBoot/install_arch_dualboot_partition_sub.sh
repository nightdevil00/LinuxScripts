...
get_user_info() {
    ...
    while true; do
        ...
        echo
        read -rs -p "Confirm your password: " password_confirm
        echo
    ...
    read -rp "Enter your desired locale (e.g., us) [us]: " locale
    locale=${locale:-us}
    read -rp "Enter your desired system language (e.g., en_US.UTF-8) [en_US.UTF-8]: " language
...
setup_system() {
    ...
    read -rp "Your timezone is detected as $timezone. Is this correct? (y/N) " confirm_tz
    if [[ ! "$confirm_tz" =~ ^[yY]$ ]]; then
        read -rp "Please enter your timezone (e.g., Europe/Bucharest): " timezone
    fi
...
partition_disk() {
    ...
    read -rp "Select a disk for installation (1, 2, ...): " disk_num
    ...
    if $has_efi && $has_ntfs; then
        ...
        case $choice in
            1)
                ...
                read -rp "Enter the size for the EFI partition (e.g., 2200M) [2200M]: " efi_size
                efi_size=${efi_size:-2200M}
                read -rp "Enter the size for the ROOT partition (e.g., 30G) [30G]: " root_size
            ...
            2)
                ...
                read -rp "Found ${free_space_size_mb}MB of free space starting at ${free_space_start}MB. Do you want to install Arch Linux in this space? (y/N) " install_in_free
        ...
        read -rp "This will format the entire disk $disk. All data will be lost. Are you sure? (y/N) " confirm_format
...
main() {
    ...
    read -rp "Installation finished in $installation_time seconds. Do you want to chroot into the new system? (If you say no, the system will reboot) (y/N) " chroot_choice
