...
partition_disk() {
    ...
    if [[ -n "$WIN_EFI_DEV" ]]; then
        echo "💠 Windows EFI detected on $WIN_EFI_DEV → leaving untouched"
        ...
        # Create Arch EFI + root
        parted --script "$TARGET_DISK" mkpart ESP fat32 "$EFI_START" "$EFI_END"
        parted --script "$TARGET_DISK" set "$(parted -m "$TARGET_DISK" print | tail -n1 | cut -d: -f1)" esp on
    ...
    # Detect partitions
    EFI_DEV=$(lsblk -ln -o PATH,PARTTYPE "$TARGET_DISK" | awk '$2=="ef00"{p=$1} END{print p}')
    ROOT_DEV=$(lsblk -ln -o PATH,PARTTYPE "$TARGET_DISK" | awk '$2=="8300"{p=$1} END{print p}')

    if [[ "$TARGET_DISK" == *nvme* ]]; then
        EFI_PART_NUM="${EFI_DEV##*p}"
    else
        EFI_PART_NUM="${EFI_DEV##*[^0-9]}"
    fi

    echo "✅ Partitions ready: EFI=$EFI_DEV (part $EFI_PART_NUM), ROOT=$ROOT_DEV"
}
