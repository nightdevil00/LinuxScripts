...
BACKGROUND_SYMLINK="$HOME/.config/hypr/background"
...
SYMLINK_PATH="$HOME/.config/hypr/background"
...
while true; do
    ...
    if [ -f "$WALLPAPER_PATH" ]; then
        ...
        matugen image "$WALLPAPER_PATH" --config "$CONFIG_FILE" --prefer saturation
...
INITIAL_WALLPAPER_PATH=$(readlink -f "$BACKGROUND_SYMLINK")
if [ -f "$INITIAL_WALLPAPER_PATH" ]; then
    echo "Initial wallpaper: $INITIAL_WALLPAPER_PATH"
    matugen image "$INITIAL_WALLPAPER_PATH" --config "$MATUGEN_CONFIG_DIR/config.toml" --prefer saturation
    echo "Initial matugen run complete."
else
    echo "Could not determine initial wallpaper from $BACKGROUND_SYMLINK. Skipping initial matugen run."
fi
